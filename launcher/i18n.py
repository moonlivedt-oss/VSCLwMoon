...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "тяжёлые серверы не грузятся, память свободна.":
            "Open the editor with just the stacks you need — the other "
            "heavy servers stay unloaded and memory stays free.",
        "Не найден CLI VS Code (code.cmd). Добавь его в PATH.":
            "VS Code CLI (code.cmd) not found. Add it to PATH.",
        "Тёмная": "Dark",
        "Светлая": "Light",
        "тяжёлый": "heavy",
        "средний": "medium",
        ...
        "Обновить": "Refresh",
        "Обновить замер памяти запущенного VS Code":
            "Re-measure memory of running VS Code",
        "VS Code сейчас: замеряю…": "VS Code now: measuring…",
        "VS Code сейчас: {mb} МБ, {n} процессов":
            "VS Code now: {mb} MB, {n} processes",
        "VS Code сейчас не запущен": "VS Code is not running",
        "Суммарный working set всех процессов VS Code":
            "Total working set of all VS Code processes",
        # обновления (#8)
        "Доступна новая версия {ver} — открыть страницу релизов":
            "New version {ver} available — open releases page",
        "Показать": "Show",
        # пресеты
        "Пресет": "Preset",
        "Сохранить…": "Save…",
        ...
        "Импортировать пресеты из файла (объединяются с текущими)":
            "Import presets from a file (merged with current ones)",
        ...
        "пресетом одним двойным кликом (без окна лаунчера)":
            "Create a .cmd file that opens VS Code with the chosen "
            "preset in one double-click (no launcher window)",
        "Сначала выбери пресет в списке — ярлык открывает VS Code "
        "с ним.":
            "Pick a preset in the list first — the shortcut opens VS Code "
            "with it.",
        ...
        "На выбор не влияет.":
            "Filters cards by title, note and extension ids. "
            "Does not change the selection.",
        # автодетект (#1)
        "Похоже на проект: {stacks}. Включить эти стеки?":
            "Looks like a {stacks} project. Enable these stacks?",
        "Включить": "Enable",
        "Скрыть": "Dismiss",
        "Включены стеки по типу проекта: {stacks}":
            "Enabled stacks by project type: {stacks}",
        # папка проекта
        ...
        "Закрыть VS Code перед стартом (чтобы память освободилась)":
            "Close VS Code before start (to actually free memory)",
        "   Мягко: дать VS Code сохранить (иначе принудительно)":
            "   Soft: let VS Code save (otherwise forced)",
        "Открыть в новом окне (--new-window)": "Open in a new window (--new-window)",
        "Без GPU-ускорения (--disable-gpu) — для слабых видеокарт":
            "No GPU acceleration (--disable-gpu) — for weak GPUs",
        "Голый режим: полностью без расширений (--disable-extensions)":
            "Bare mode: no extensions at all (--disable-extensions)",
        "Профиль": "Profile",
        "имя существующего профиля VS Code (необязательно)":
            "name of an existing VS Code profile (optional)",
        # настройка VS Code
        "НАСТРОЙКА VS CODE": "VS CODE SETUP",
        "Автонастройка settings.json": "Auto-configure settings.json",
        "Добавить рекомендованные настройки для установленных стеков":
            "Add recommended settings for installed stacks",
        # нижняя панель
        "Показать команду": "Show command",
        "Запустить VS Code": "Launch VS Code",
        "Что выключится": "What gets disabled",
        "Показать список расширений, которые будут выключены":
            "Show the list of extensions that will be disabled",
        "Здесь появится итоговая команда и статус запуска.":
            "The resulting command and launch status will appear here.",
        # summary
        "Голый режим: все расширения выключены (--disable-extensions).":
            "Bare mode: all extensions disabled (--disable-extensions).",
        "Список расширений не получен (нет CLI?).":
            "Extension list not available (no CLI?).",
        "Включено {en}, выключено {dis} — экономия ~{saved} МБ":
            "Enabled {en}, disabled {dis} — saving ~{saved} MB",
        ...
        "чего нет в карте, останется включённым.":
            "{n} extensions from unselected stacks will be disabled. always_on "
            "and anything not in the map stays enabled.",
        "Ничего не выключается — всё установленное останется включённым.":
            "Nothing gets disabled — everything installed stays enabled.",
        "Копировать": "Copy",
        ...
        "что внутри стека и установка/удаление.":
            "Check the stacks you need today. The left strip is memory load: "
        ...
        "в этой сессии).":
            "The checkbox turns this stack ON in the VS Code you launch. "
        ...
        "маркетплейс, установка и удаление.":
            "What extensions the stack has and why: a description of each, a "
            "marketplace link, install and remove.",
        "Установлено {inst} из {total} расширений стека. Выключение стека "
        "коснётся только этих установленных.":
            "{inst} of {total} stack extensions installed. Disabling the stack "
            "affects only these installed ones.",
        "нет": "none",
        "Ни одно из {total} расширений стека не установлено.":
            "None of the {total} stack extensions are installed.",
        "Расширения этого стека не установлены — галочка ни на что не влияет. "
        "Поставить их можно в «Подробнее» → «Установить недостающие».":
            "This stack's extensions aren't installed — the checkbox has no "
        ...
        "когда он выключен и сегодня не нужен.":
            "Heavy stack: language servers and analyzers keep hundreds of MB "
        ...
        "включённым для своих языков, выключайте на чужих проектах.":
            "Medium stack: noticeable but moderate memory use. Keep it on for "
        ...
        "включённым — на экономию он влияет мало.":
            "Light stack: barely affects memory. Fine to keep on — it changes "
        ...
        "в этой сессии.":
            "Below are the stack's extensions and what each does. "
        ...
        "выключение освобождает ~{mb} МБ":
            "{total} extensions · {n} installed · {load} load · disabling "
            "frees ~{mb} MB",
        "установлено": "installed",
        "нет в системе": "not installed",
        "Расширение установлено — грузится в VS Code, пока стек включён.":
            "Extension installed — loads in VS Code while the stack is on.",
        "Расширения нет на диске. «Установить» скачает его из маркетплейса.":
            "Not on disk. “Install” downloads it from the marketplace.",
        "Маркетплейс ↗": "Marketplace ↗",
        "Открыть страницу расширения в маркетплейсе VS Code — описание, автор, "
        "рейтинг и что оно запрашивает — перед установкой.":
            "Open the extension's VS Code marketplace page — description, "
            "author, rating and what it requests — before installing.",
        "Скачать и установить это расширение из маркетплейса VS Code.":
            "Download and install this extension from the VS Code marketplace.",
        "Удалить расширение с диска. Переустановить можно кнопкой «Установить».":
            "Remove the extension from disk. Reinstall with the Install button.",
        "Описание не задано — открой «Маркетплейс», чтобы прочитать, что "
        "делает расширение.":
            "No description set — open Marketplace to read what the extension "
            "does.",
        "Установить": "Install",
        "Подробнее": "Details",
        "Отметить все стеки (с учётом фильтра поиска)":
            "Check all stacks (respecting the search filter)",
        "Снять все галочки — останется только ядро (always_on) и незамапленные "
        "расширения":
            "Uncheck all — only the core (always_on) and unmapped extensions "
            "remain",
        # per-extension оверрайды (#9)
        "по стеку": "by stack",
        "всегда вкл": "always on",
        "всегда выкл": "always off",
        "Поведение этого расширения независимо от галочки стека":
            "Behavior of this extension regardless of the stack checkbox",
        ...
        "Отмена": "Cancel",
        "Отмена…": "Cancelling…",
        "Прервать пакетную установку (текущее расширение доустановится, следующие — нет)":
            "Stop the batch install (the current extension finishes, the rest do not)",
        ...
        "прописать их в PATH.":
            "Install compilers and SDKs (C/C++, Java, Go, Rust…) via winget and "
            "add them to PATH.",
        ...
        "установки откройте новый терминал, чтобы PATH подхватился.":
            "VS Code extensions add highlighting and hints, but they can't build "
        ...
        "установлено ✓ — перезапустите терминал":
        ...
        "занять несколько минут. Продолжить?":
        ...
        "Настройка VS Code": "VS Code setup",
        "Прописать путь к компилятору в settings.json VS Code, чтобы IntelliSense "
        "и сборка заработали.":
            "Write the compiler path into VS Code settings.json so IntelliSense "
            "and builds work.",
        "Даёт: {tools}": "Provides: {tools}",
        "Добавить в PATH": "Add to PATH",
        "Компилятор найден на диске — добавить его каталог в PATH без повторной "
        "загрузки.":
            "Compiler found on disk — add its folder to PATH without downloading "
            "again.",
        "Поставить": "Install",
