...
- **Языки и инструменты** — кнопка в шапке списка стеков открывает управление самими тулчейнами: компилятор C/C++ (MinGW-w64 + GDB + make, опционально CMake/Ninja/Clang), JDK, Go, Rust, .NET, Node.js, Ruby, Python, PHP, Lua, Deno, Bun, Zig, Dart, Julia, Swift, Git. Расширение VS Code для языка бесполезно без него — здесь недостающее ставится в пару кликов, а не гуглится по инструкциям.
  <details>
  ...
  - **«Настроить VS Code»** прописывает путь к тулчейну в `settings.json` (только недостающие ключи, с бэкапом): для C/C++ — `C_Cpp.default.compilerPath` и разумные стандарты (`c17`/`c++20`); для Python — `python.defaultInterpreterPath` (чтобы не выбирать интерпретатор вручную на первом запуске);
  - **«Проверить окружение»** (доктор) показывает установленные тулчейны с версиями, здоровье PATH **раздельно для пользовательского и системного** (дубли, мёртвые записи, длина) и `JAVA_HOME` — и тут же чинит: **«Почистить свой PATH»** убирает дубли и несуществующие каталоги из пользовательской ветки (без прав администратора), **«Почистить системный PATH»** — из машинной (поднимает UAC; если лаунчер уже elevated — пишет напрямую). Обе чистки не удаляют резервные каталоги тулчейнов (`go\bin`, `.dotnet\tools`, а также любые `path_hints` из каталога) и делают бэкап в файл рядом с конфигом. **«Исправить JAVA_HOME»** находит установленный JDK, прописывает переменную и, если JDK не на PATH, дописывает его `bin`. После установки JDK `JAVA_HOME` выставляется автоматически;
  - детект «неполной» установки: у пакета можно задать команду-подтверждение (`verify_cmd`) — например, `.NET` считается установленным только если `dotnet --list-sdks` реально выдаёт SDK, а не когда в PATH просто есть мультиплексор рантайма без SDK;
...
python vscode_launcher.py --configure-vscode cpp        # компилятор C++ → settings.json VS Code
python vscode_launcher.py --configure-vscode python     # интерпретатор Python → settings.json VS Code
python vscode_launcher.py --doctor                      # окружение: тулчейны, здоровье PATH, JAVA_HOME
python vscode_launcher.py --clean-path                  # предпросмотр: дубли и мёртвые записи в вашем PATH
python vscode_launcher.py --clean-path --yes            # применить чистку пользовательского PATH (с бэкапом)
